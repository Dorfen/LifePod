...
Screen::Screen()
    : event_({7 * LINES / 10, 7 * COLS / 10, 0, 0, true, ACS_VLINE, ACS_HLINE, true,
              "Event"}),
      cmd_({3 * LINES / 10 + 1, 7 * COLS / 10, 7 * LINES / 10, 0, true, ACS_VLINE,
            ACS_HLINE, true, "Command"}),
      status_({LINES, 3 * COLS / 10, 0, 7 * COLS / 10, true, ACS_VLINE, ACS_HLINE, true,
               "Status"})
{
}
...
void Screen::displayShipStatus(const Ship &s, const bool r)
{
    ...
    printLoadbar(std::pair<int, int>(3, 1), s.getSystemValue<Ship::System::Colon>(),
                 1000);
    status_.print(4, 1, "%04i | 1000", s.getSystemValue<Ship::System::Colon>());
    printLoadbar(std::pair<int, int>(7, 1), s.getSystemValue<Ship::System::Landing>(),
                 100);
    status_.print(8, 1, "%03i | 100", s.getSystemValue<Ship::System::Landing>());
    ...
    printLoadbar(std::pair<int, int>(27, 1), s.getSystemValue<Ship::System::Water>(),
                 100);
    ...
    printLoadbar(std::pair<int, int>(11, 1), s.getSystemValue<Ship::System::Build>(),
                 100);
    status_.print(12, 1, "%03i | 100", s.getSystemValue<Ship::System::Build>());
    if (r == true) status_.refresh();
}
...
void Screen::printShip(const bool r)
{
    ...
    event_.print(y - 14, x - 63,
                 "                    __,' __`.                _..----....____");
    event_.print(y - 13, x - 63,
                 "        __...--.'``;.   ,.   ;``--..__     .'    ,-._    _.-'");
    event_.print(y - 12, x - 63,
                 "  _..-''-------'   `'   `'   `'     O ``-''._   (,;') _,'");
    event_.print(y - 11, x - 63,
                 ",'________________                          \\`-._`-','");
    event_.print(y - 10, x - 63, " `._              ```````````------...___   '-.._'-:");
    event_.print(y - 9, x - 63,
                 "    ```--.._      ,.                     ````--...__\\-.");
    event_.print(y - 8, x - 63,
                 "            `.--. `-`                       ____    |  |`");
    event_.print(y - 7, x - 63,
                 "              `. `.                       ,'`````.  ;  ;`");
    event_.print(y - 6, x - 63,
                 "                `._`.        __________   `.      \\'__/`");
    event_.print(y - 5, x - 63,
                 "                   `-:._____/______/___/____`.     \\  `");
    event_.print(y - 4, x - 63, "                              |       `._    `.    \\");
    event_.print(y - 3, x - 63,
                 "                               `._________`-.   `.   `.___");
    event_.print(y - 2, x - 63,
                 "                                                  `------'`");
    if (r == true) event_.refresh();
}

void Screen::printLoadbar(const std::pair<int, int> coord, const int value, const int max,
                          const bool r)
{
    ...
    bar[bar_size] = '\0';
    for (auto &i: bar) i = '-';
    for (int i = 0; i < coord_ && bar[i] != '\0'; i++) bar[i] = '#';
